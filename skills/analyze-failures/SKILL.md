...
4. **Compare failed against passing.** For each failed run and for the baseline run, first call `sipfront_stats_types_for_run` and fetch the conversation and audio measures with `sipfront_stats_for_run` (transcripts, turn and latency metrics, MOS, jitter, packet loss), then `sipfront_siptrace_for_run` and `sipfront_callstates_for_run`. Evaluate the conversation first, then signaling and media. Compare each failed run with the baseline and look for:
